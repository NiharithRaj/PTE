...
class HIWViewController: UIViewController {
    var viewModel: HIWViewModel!
    let count = 25.0
    let recordTime = 6
    var answerCount = 10
    let speech = Speech()
...
//    @IBOutlet weak var answerView: UIView!
    ...
    private func displayCorrectAnswer() {
        let speech = viewModel.model.answer
        var ranges:[NSRange] = []
        let attri = NSMutableAttributedString(string: speech)

        for index in 0..<viewModel.model.highlightingWords.count {
            if let tupple = speech.replaceFirst(of: viewModel.model.both[index]), let slashrange = viewModel.model.both[index].replaceFirst(of: "/") {
                attri.addAttributes([NSAttributedString.Key.backgroundColor : UIColor.yellow], range:NSRange(location: tupple.location, length: slashrange.location))
                attri.addAttributes([NSAttributedString.Key.foregroundColor : rgb(r: 0, g: 122, b: 255)], range:NSRange(location: tupple.location + slashrange.location + 1, length: viewModel.model.both[index].count - slashrange.location))
                ranges.append(tupple)
            }
        }
...
public func replaceFirst(of pattern:String) -> NSRange? {
  if let range = self.range(of: pattern){
    var range1 = NSRange(range, in: self)
    return range1
  }else{
    return nil
  }
