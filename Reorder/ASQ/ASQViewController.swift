...
class ASQViewController: UIViewController {
    let speech = Speech()
    var viewModel: ASQViewModel!
...
struct ASQViewModel {
    ...
    let total:Int
    init(model: ASQ, questionNumber:Int, total:Int) {
        self.model = model
    ...
    var speechTime:Int {
        var time = 5
        let arr = model.question.components(separatedBy: CharacterSet.init(charactersIn: " "))
        if arr.count < 7 {
            time = 4
        } else if arr.count > 10 && arr.count <= 13 {
            time = 5
        } else if arr.count > 13 && arr.count <= 15 {
            time = 6
